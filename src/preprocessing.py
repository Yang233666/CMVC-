...
import numpy as np
from src.helper import checkFile, invertDic, getEmbeddings
    ...
    def process(self):
        ...
        if not checkFile(fname1) or not checkFile(fname2):
            ...
            for triple in self.triples:  # Get all subject, objects and relations
                ...
                if 'OPIEC59k' in self.p.dataset:
                    ent1_truelink.append(triple['subject_wiki_link'])
                    ent2_truelink.append(triple['object_wiki_link'])
                elif 'reverb45k' in self.p.dataset:
                    ent1_truelink.append(triple['true_link']['subject'])
                    ent2_truelink.append(triple['true_link']['object'])
                relList.append(rel)
                ent2List.append(obj)
                ...
                if not isinstance(triple['src_sentences'], list):
                    triple['src_sentences'] = [triple['src_sentences']]
            ...
            for i in range(len(ent1List)):
                ...
                if sub in assume_sub.keys():
                    ...
                    assume_sub[sub] = [i]
                if 'NYTimes2018' not in self.p.dataset:
                    truelink = ent1_truelink[i]
                    if sub in ent_truelink:
                        ent_truelink[sub].append(truelink)
                    else:
                        ent_truelink[sub] = [truelink]
            ...
            for i in range(len(ent2List)):
                ...
                if obj in assume_obj.keys():
                    ...
                    assume_obj[obj] = [i]
                if 'NYTimes2018' not in self.p.dataset:
                    truelink = ent2_truelink[i]
                    if obj in ent_truelink:
                        ent_truelink[obj].append(truelink)
                    else:
                        ent_truelink[obj] = [truelink]

            if 'NYTimes2018' not in self.p.dataset:
                self.ent2truelink_set, self.ent2truelink = dict(), dict()
                for ent, truelinks in ent_truelink.items():
                    self.ent2truelink[ent] = Counter(truelinks).most_common()[0][0]
                    self.ent2truelink_set[ent] = set(truelinks)
            ...
            if 'NYTimes2018' not in self.p.dataset:
                pickle.dump(self.ent2truelink, open(self.folder_to_make+'self.ent2truelink','wb'))
                pickle.dump(self.ent2truelink_set, open(self.folder_to_make + 'self.ent2truelink_set', 'wb'))
        else:
            ...
            # if 'NYTimes2018' not in self.p.dataset:
            #     self.ent2truelink = pickle.load(open(self.folder_to_make+'self.ent2truelink', 'rb'))
            #     self.ent2truelink_set = pickle.load(open(self.folder_to_make + 'self.ent2truelink_set', 'rb'))
        ...
        if self.p.use_Entity_linking_dict:
            ...
            if not checkFile(fname1) or self.p.change_EL_threshold:
                ...
                print('load el ent_old_id2new_id')
