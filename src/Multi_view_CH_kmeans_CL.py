...
def multi_view_centers_dense(X_view_1, X_view_2, sample_weight, labels_view_1, labels_view_2, n_clusters, distances):
    ...
    for i in range(n_samples):
        c1 = labels_view_1[i]
        c2 = labels_view_2[i]
        if c1 == c2:  # only those examples are included that both views agree on: 2138->1943
    ...
    weight_in_cluster[np.where(weight_in_cluster == 0.0)] = 1
...
def ch_cluster(X, lables, centers=None, dataset=""):
    cluster = dict()
    for idx, i in enumerate(lables):
        if i in cluster:
            cluster[i].append(idx)
        else:
            cluster[i] = [idx]
    ...
    if 'reverb' in dataset:
        ...
        if centers is None:
            n_cluster = [10, 20, 490, 1600, 2300, 3898, 5700, 6700][
                np.argmin(np.abs(np.array([10, 20, 490, 1600, 2300, 3898, 5700, 6700]) - len(cluster)))]
        else:
            n_cluster = [10, 20, 490, 1600, 2300, 3898, 5700, 6700][
                np.argmin(np.abs(np.array([10, 20, 490, 1600, 2300, 3898, 5700, 6700]) - len(centers)))]

    res = np.zeros(n_cluster)
    mean_X = np.mean(X, axis=0)
    for id, idx_list in cluster.items():
        cluster_i = X[idx_list]
        if centers is None:
            mean_k = np.mean(cluster_i, axis=0)
        else:
            mean_k = centers[id]
        res[id] = len(cluster_i) * np.sum((mean_k - mean_X) ** 2)
        if len(cluster_i) > 1 and np.sum((cluster_i - mean_k) ** 2) > 1e-6:
            res[id] /= np.sum((cluster_i - mean_k) ** 2)
    res = res * (X.shape[0] - len(cluster)) / (len(cluster) - 1.0)
    res[res == 0.0] = 1
    return res


def multi_view_labels_inertia_precompute_dense(X_view_1, X_view_2, centers_view_1, centers_view_2, labels_view_1,
                                               labels_view_2):
...
def multi_view_labels_inertia(X_view_1, X_view_2, sample_weight, x_view_1_squared_norms, x_view_2_squared_norms,
                              ...
                              labels_view_1=None, labels_view_2=None):
...
def multi_view_spherical_kmeans_single_lloyd(
    ...
    return best_labels, best_inertia, i + 1, \
           (best_centers_view_1, best_centers_view_2), np.concatenate(
        [labels_view_1.reshape(1, -1), labels_view_2.reshape(1, -1)], axis=0)
...
def multi_view_spherical_k_means(
    ...
    if n_jobs == 1:
        ...
        for it in range(n_init):
            # run a k-means once
            labels, inertia, n_iter_, center_view, lable_view = multi_view_spherical_kmeans_single_lloyd(
                X_view_1,
                X_view_2,
                n_clusters,
                sample_weight,
            ...
            ans[it, 0] = inertia
            t1 = CE(
                np.matmul(X_view_1, center_view[0].transpose()) * calinski_harabasz_score(X_view_1, lable_view[0]),
                np.matmul(X_view_2, center_view[1].transpose()) * calinski_harabasz_score(X_view_2, lable_view[1])
            )
            t2 = CE(
                np.matmul(X_view_2, center_view[1].transpose()) * calinski_harabasz_score(X_view_2, lable_view[1]),
                np.matmul(X_view_1, center_view[0].transpose()) * calinski_harabasz_score(X_view_1, lable_view[0])
            ...
            ans[it, 1] = t1 + t2
            ch_0 = ch_cluster(X_view_1, lable_view[0, :], center_view[0])
            ch_1 = ch_cluster(X_view_2, lable_view[1, :], center_view[1])
            ans[it, 2] = BhattacharyyaDistance(
                np.matmul(np.matmul(X_view_1, center_view[0].transpose()), ch_0),
                np.matmul(np.matmul(X_view_2, center_view[1].transpose()), ch_1)
            )

            ans[it, 3] = cluster_test(p, side_info, labels, true_ent2clust, true_clust2ent, False)[2]
            # print()
            # ans[it, 3] = cluster_test(labels, true_ent2clust, true_clust2ent, False)[2]
            print(it, ':', ans[it])
            ...
            if best_inertia is None or inertia < best_inertia:
                ...
                best_n_iter = n_iter_
                best_center_view = (center_view[0].copy(), center_view[1].copy())
                best_label_view = lable_view.copy()
    else:
        ...
        labels, inertia, n_iters, center_view, lable_view = zip(*results)
        best = np.argmin(inertia)
        best_labels = labels[best]
        best_inertia = inertia[best]
        best_n_iter = n_iters[best]
        best_center_view = center_view[best]
...
class Multi_view_SphericalKMeans(object):
    ...
    def fit(self, X_view_1, X_view_2, sample_weight=None):
        ...
        self.labels_, self.inertia_, self.n_iter_, self.centers_view, self.labels_view = multi_view_spherical_k_means(
            X_view_1,
            X_view_2,
